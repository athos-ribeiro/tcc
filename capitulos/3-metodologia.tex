...
A proposta fora elaborada a partir das experiências do autor com as avaliações\index{avaliações} de ferramentas\index{ferramenta} referentes ao SATE V ao longo do ano de 2014, em especial à \textit{Ockham\index{ockham} Criteria} e dos estudos das metodologias aplicadas por \cite{harvard} e \cite{nsa}, sendo que o trabalho aqui apresentado trata-se de uma proposta de generalização dos casos estudados.

\section{Refinamento da Proposta e Definição das Etapas}

As etapas foram definidas através do desenvolvimento iterativo do fluxograma apresentado na Seção \nameref{metodologia_proposta:visao_geral} do Capítulo \nameref{metodologia_proposta} e refinadas através de análises\index{análise} de grafos, onde pôde-se observar a dependência entre cada uma das atividades do processo para que as mesmas fossem ordenadas e possivelmente paralelizadas de forma correta.

\section{Aplicação da Abordagem em Estudos de Caso}

Para a aplicação da abordagem, buscou-se sempre trabalhar com software de licenças Livres, fosse para ferramentas\index{ferramenta} de análise estática, suítes de teste ou ferramentas\index{ferramenta} para auxiliar a extração e análise de resultados, de modo a melhor compreender as ferramentas\index{ferramenta} utilizadas quando necessário e evitar quaisquer irregularidades relacionadas ao uso indevido de ferramentas\index{ferramenta}\footnote{Algumas licenças não permitem que se investigue, por exemplo, a execução de uma ferramenta.}.

Para as análises\index{análise} de resultados, utilizaram-se apenas algumas ferramentas\index{ferramenta} do projeto GNU\footnote{\url{http://gnu.org}} como \textit{bash}, \textit{grep}, \textit{comm} e \textit{sort} dado que não são necessárias ferramentas\index{ferramenta} mais sofisticadas ao se trabalhar com conjuntos\footnote{Note que as análises\index{análise} da abordagem aqui proposta tratam de manipulações de conjuntos.} em formatos de texto.
